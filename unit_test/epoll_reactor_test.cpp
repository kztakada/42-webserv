...
TEST(EpollFdEventReactor, ReturnsReadEventWhenPipeReadable)
{
    ...
    Result<void> add_r = reactor.addWatch(watch);
    ASSERT_TRUE(add_r.isOk());

    const char b = 'x';
    ASSERT_EQ(1, write(write_fd.get(), &b, 1));

    Result<const std::vector<server::FdEvent>&> wait_r =
...
TEST(EpollFdEventReactor, ReturnsWriteEventWhenPipeWritable)
{
    ...
    Result<const std::vector<server::FdEvent>&> wait_r = reactor.waitEvents(0);
    ASSERT_TRUE(wait_r.isOk());
...
TEST(EpollFdEventReactor, RemoveWatchStopsReceivingEvents)
{
    ...
    Result<const std::vector<server::FdEvent>&> wait_r = reactor.waitEvents(20);
    ASSERT_TRUE(wait_r.isOk());
...
TEST(EpollFdEventReactor, RejectsInvalidWatchEvents)
{
    ...
    Result<void> r = reactor.addWatch(bad);
...
{
    // 1) Pick a local port that is very likely closed: bind+listen to an
    // ephemeral port, then close it so there is no listener.
    int listen_fd_raw = socket(AF_INET, SOCK_STREAM, 0);
    ASSERT_GE(listen_fd_raw, 0);
    ScopedFd listen_fd(listen_fd_raw);

    int yes = 1;
    ASSERT_EQ(0, setsockopt(listen_fd.get(), SOL_SOCKET, SO_REUSEADDR, &yes,
                     sizeof(yes)));
    ...
    ASSERT_EQ(
        0, bind(listen_fd.get(), reinterpret_cast<sockaddr*>(&listen_addr),
               sizeof(listen_addr)));
    ASSERT_EQ(0, listen(listen_fd.get(), 1));

    sockaddr_in bound_addr;
    socklen_t bound_len = sizeof(bound_addr);
    ASSERT_EQ(0, getsockname(listen_fd.get(),
                     reinterpret_cast<sockaddr*>(&bound_addr), &bound_len));
    const uint16_t port = ntohs(bound_addr.sin_port);
    ASSERT_NE(0, port);

    ASSERT_EQ(0, close(listen_fd.release()));

    // 2) Create a non-blocking client socket and watch it for "write" to learn
    // connect result.
    ...
    int c = connect(
        client_fd.get(), reinterpret_cast<sockaddr*>(&dest), sizeof(dest));
    if (c != 0)
    {
        ASSERT_TRUE(errno == EINPROGRESS || errno == ECONNREFUSED);
    }

    // 4) Expect EPOLLERR to be surfaced as kErrorEvent (and to suppress
    // kWriteEvent).
    Result<const std::vector<server::FdEvent>&> wait_r =
    ...
    ASSERT_EQ(0, getsockopt(client_fd.get(), SOL_SOCKET, SO_ERROR, &so_error,
                     &so_error_len));
