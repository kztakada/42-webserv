...
SessionContext::SessionContext(int fd, const SocketAddress& server_addr,
    ...
    : request(),
      ...
      pause_write_until_body_ready(false),
      body_watch_fd(-1),
      body_watch_read(false),
      cgi_stdout_fd_for_response(-1),
      cgi_prefetched_body(),
      waiting_cgi_first_body(false),
      waiting_cgi_first_body_start(0),
      in_read_backpressure(false),
      in_write_backpressure(false),
      has_request_start_time(false),
      request_start_time_seconds(0),
