...
SessionContext::SessionContext(int fd, const SocketAddress& server_addr, const SocketAddress& client_addr,
                   FdSessionController& ctl, const RequestRouter& rt)
    : request(), response(),
      socket_fd(fd, server_addr, client_addr),
      recv_buffer(), send_buffer(),
      controller(ctl), router(rt),
      body_source(NULL), response_writer(NULL),
      current_state(NULL), pending_state(NULL),
