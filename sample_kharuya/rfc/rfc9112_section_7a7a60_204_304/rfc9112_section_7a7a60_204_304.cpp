...
static bool ensureDeleteMeTxtExists()
{
    std::ofstream ofs("www/delete_me.txt");
    if (!ofs)
    {
        return false;
    }
...
int main()
{
    ...
    const int expected_status = 204;
    const char* test_name = "rfc9112_section_7a7a60_204_304";
    ...
    if (!ensureDeleteMeTxtExists())
    {
        std::cerr << "FAIL: cannot create www/delete_me.txt\n";
        return 1;
    }
